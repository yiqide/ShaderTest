...
Shader "MyShader/SurfaceShader2.0"
{
    Properties
    {
        ...
        SubShader
    {
        pass
        {
            ...
            put vert(Input input)
            {
                ...
                float3 worldNormal=normalize(mul(input.normal,(float3x3)unity_WorldToObject));//把法线从对象空间转换到世界空间
                float3 worldLight=normalize(_WorldSpaceLightPos0.xyz);//_WorldSpaceLightPos0光源方向
                //saturate如果x<0返回0,如果x>1返回1,否则返回x. dot数量积
                float3 diffuse= _LightColor0.rgb*_Diffuse.rgb*saturate(dot(worldNormal,worldLight));//_LightColor0光源的颜色和强度
