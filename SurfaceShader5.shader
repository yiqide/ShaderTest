        ...
        SubShader
    {
        pass
        {
            ...
            put vert(Input input)
            {
                ...
                o.uv=input.texcoord.xy*_MainTex_ST.xy+_MainTex_ST.zw;
                return o;
            }

            float4 frag(put output) :SV_Target//把用户的输出颜色存到一个渲染目标（render target），这里输出到默认的帧缓存中
            {
                float3 albedo =tex2D(_MainTex,output.uv)*_Diffuse;
                ...
                float3 diffuse= _LightColor0.rgb*albedo*_Diffuse.rgb*saturate(dot(output.worldNormal,worldLightDir));
